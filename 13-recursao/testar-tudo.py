...
if len(sys.argv) < 2:
	print(f"Uso: python {os.path.basename(__file__)} labXX.py")
	sys.exit()
...
in_file = "testes/" + test_name + "-in.txt"

while (os.path.exists(in_file)):
	out_file = "testes/" + test_name + "-out.txt"
	if not os.path.exists(out_file):
		print("Arquivo", out_file, "não encontrado.")
		sys.exit()

	ans_file = "testes/" + test_name + "-ans.txt"
	diff_file = "testes/" + test_name + "-diff.txt"
